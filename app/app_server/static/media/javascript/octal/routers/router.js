 ...
 */
define(["backbone", "jquery", "agfk/views/explore-view", "agfk/views/learning-view", "agfk/views/quiz-view", "agfk/views/apptools-view", "agfk/views/loading-view", "agfk/models/app-model", "agfk/utils/errors", "agfk/views/error-view"],
  ...
  return (function(){
    ...
    pvt.routeConsts = {
      ...
      pLearnMode: "learn",
      pQuizMode: "quiz", /////////////HEY I CHANGED THIS - ZACK
      ...
      qViewId: "quiz-view-wrapper",  //id for main quiz view div ----ZACK
    ...
    pvt.cleanUpViews = function(){
      ...
      // Adding a quiz view --Zack
      if (thisRoute.qview instanceof ExploreView){
        thisRoute.qview.close();
        thisRoute.qview = undefined;
      }
    };
    ...
    return Backbone.Router.extend({
      ...
      nodeRoute: function(nodeId, paramsObj) {
        ...
        if (doRender){
         ...
         // async start loading Viz before the view, else load after the view
        var loadViz = typeof window.Viz === "undefined" && window.vizPromise === undefined,
            preLoadViz = (paramsObj[qViewMode] === pExploreMode || paramsObj[qViewMode] === pQuizMode);
        ...
        function postNodePop() {
          ...
          case pQuizMode:
            if (doRender){
              thisRoute.qview = new ExploreView({model: thisRoute.appData.get("graphData"), appRouter: thisRoute});
            }
            thisRoute.showView(thisRoute.eview, doRender, "#" + routeConsts.eViewId);
            break;
          default:
